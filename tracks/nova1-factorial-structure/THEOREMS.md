...
| N1-STR-007 | proved theorem | Complement pairing gives legal distinct reciprocal pairs | `proofs/COMPLEMENT_PAIRING_LEMMA.md` |
| N1-COR-001 | proved theorem | Binary powers represent every residual in `[0,2^r-1]` | `proofs/DISTINCTNESS_AND_CORRECTION.md` |
| N1-RED-002 | proved theorem | Downward main occupancy plus correction gives exact local coverage | `proofs/DISTINCTNESS_AND_CORRECTION.md` |
...
| ID | Result label | Conclusion | Proof or source |
|---|---|---|---|
| N1-STR-014 | proved theorem | Marker-three divisors are legal, layer-distinct, and palette-disjoint | `proofs/MARKER_THREE_LATTICE_REPAIR.md` |
| N1-STR-015 | proved theorem | Main support generates exactly `3Z`; palette attains every residue modulo `3` | `proofs/MARKER_THREE_LATTICE_REPAIR.md` |
...
| N1-STR-018 | proved theorem | For `n>=120368`, every layer has at least `2^{n/(3 log n)-1}` legal high-prime cores | `proofs/MARKER_THREE_MENU_CAPACITY.md` |
...
| N1-RED-006 | proved theorem | Increasing-layer selection leaves residual below `max((2/3)^Lq,2^L)` | `proofs/QUOTIENT_ENDPOINT_SUPPORT.md` |
...
| N1-STR-022 | proved theorem | Unique-parent streaming exactly recovers connected maxima and counts | `proofs/STREAMING_CONNECTED_PREFIX_CERTIFIER.md` |
...
| N1-STR-024 | proved theorem | `Gamma_n=(P_n/Q_n)^{1/L}` meets the exact count gate iff `Gamma_n>=1` | `proofs/NORMALIZED_CONNECTED_PREFIX_SURPLUS.md` |
...
| N1-OBS-004 | proved theorem | The parity-span lower bound is optimal using only oddness, `K_t`, and `D_t` | `proofs/PARITY_SPAN_CRITERION_RECONSTRUCTION.md` |
| N1-STR-027 | proved theorem | Truncating half lists at `Y_n`, guarding multiplication by division, and checkpointing preserve the exact required product stream | `proofs/OVERFLOW_SAFE_CHECKPOINTED_MITM_STREAM.md` |

## Effective carrier identity
...
| N1-CMP-004 | finite certificate | Endpoint-support and contraction checks | `verification/ENDPOINT_SUPPORT_FINITE_REPORT.md` |
| N1-CMP-005 | finite certificate | Block and collision checks | `verification/BLOCK_COLLISION_FINITE_REPORT.md` |
| N1-CMP-006 | computational evidence | Count-surplus audit at `n=51,52,53` | `verification/connected_prefix_normalized_n51_n53.csv` |
| N1-CMP-007 | computational evidence | Count-surplus audit through `n=54` | `verification/connected_prefix_normalized_n51_n54.csv` |
| N1-CMP-008 | computational evidence | Effective count-utilization audit through `n=55` | `verification/effective_carrier_n51_n55.csv` |
| N1-CMP-009 | computational evidence | Parity-span audit through `n=56` | `verification/parity_span_effective_n51_n56.csv` |
| N1-CMP-010 | computational evidence | Effective and factorial-span audit through `n=57` | `verification/effective_carrier_n51_n57.csv` |
| N2-FIN-202 | finite certificate | Imported complete-menu coverage for `12<=n<=45` | Nova 2 commit `82603c631a106c3bff4676bdeeb9cc791fc98f3c` |
...
| N1-FIN-009 | finite certificate | Exact runtime-aware coverage at `n=54` | `verification/FULL_CORE_N54_REPORT.md` |
| N1-FIN-010 | finite certificate | Exact effective-carrier coverage at `n=55` | `verification/FULL_CORE_N55_REPORT.md` |
| N1-FIN-011 | finite certificate | Exact seven-layer coverage at `n=56` | `verification/FULL_CORE_N56_REPORT.md` |
| N1-FIN-012 | finite certificate | Exact overflow-safe dual-partition seven-layer coverage at `n=57` | `verification/FULL_CORE_N57_REPORT.md` |
...
No conclusion is asserted for `n>=58`.
...
| N1-DIS-002 | disproved route | A fixed polylogarithmic binary or ternary pool covers the half-range | `proofs/COUNTING_CAPACITY_OBSTRUCTION.md` |
...
| N1-DIS-006 | disproved route | One factorial arithmetic block per layer reaches `Y_n` | `proofs/FACTORIAL_BLOCK_CARRIER_OBSTRUCTION.md` |
| N1-DIS-007 | disproved route | Unguarded unsigned-128 half-list multiplication is an exact verifier | `proofs/OVERFLOW_SAFE_CHECKPOINTED_MITM_STREAM.md` |
| N1-DIS-008 | disproved route | The finite candidate `g_t/D_t<1.108` | `proofs/FIRST_BLOCKING_GAP_1_108_COUNTEREXAMPLE.md` |

At `n=57`, layer `3`,

\[
\frac{g_3}{D_3}=1.1674772300983786\ldots>1.108.
\]
