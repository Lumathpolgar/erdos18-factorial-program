...
int main(int argc,char**argv){try{int n=52;if(argc>1)n=std::stoi(argv[1]);auto[r,M]=pars(n);int v2=vp(n,2);std::vector<PE>f;u64 total=1;for(int p:ps(n)){if(p==2)continue;int e=vp(n,p)-(p==3);if(e>0){f.push_back({p,e,u64(e+1)});total*=u64(e+1);}}
