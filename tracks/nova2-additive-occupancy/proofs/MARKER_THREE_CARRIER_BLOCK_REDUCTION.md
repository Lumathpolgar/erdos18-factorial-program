...
For an integer `W>=0`, a set `S subseteq Z_{
>=0}` is called downward `W`-dense through `E` if
...
be downward `W`-dense through `E`, and assume both endpoints
...
If the scaled gaps satisfy

\[
a(u_{i+1}-u_i)\le E+W+1
\]

for every `i`, then `T` is downward `W`-dense through
...
The unscaled displayed condition is redundant when `a=1`; the operative condition is the scaled one.
...
Consider consecutive translates. Let
...
Thus no downward gap larger than `W` occurs between consecutive translated blocks. Induction over `i` proves downward `W`-density through `H_k=E+a u_k`. The point `H_k` belongs to the last translate because `E in S`. `QED`
...
Choose a finite ordered core chain
...
The proof applies N2-ADD-119 with `S` equal to the previous-layer sumset and with one translate for every allowed carrier core in the chosen chain.

## Exact connected-core formulation

Set `E_0=0`. For each layer `t`, define the admissible core-gap threshold
...
by joining two ordered consecutive core values when their difference is at most `D_t(E_{t-1})`.

Let `u_t^*` be any core in the connected component of `0`. Then one may set
...
This criterion is exact for the carrier-block proof engine. Failure of the criterion does not prove failure of the full rainbow sumset, because other non-block interactions between layers may still bridge a gap.

## N2-ADD-120: conditional marker-three chain theorem

### Statement

Assume the structural hypotheses of `N1-CON-003`. Suppose there exist core choices `u_t^*` generated by the connected-core recursion above for every `1<=t<=M_n`, producing endpoints
...
## Recovery and slight extension of the known initial range
...
Hence the quotient windows are occupied not only through `E_{M_n}`, but through
...
It also provides an exact falsification target for Nova 4:

1. generate or certify core menus;
2. compute connected components under the exact thresholds;
3. return the first layer and first core gap that stops the carrier-block recursion;
...
The historical Phase 12P obstruction applies only if its exact hypotheses match this carrier recursion. A future promotion must either:

1. prove that the marker-three menu sizes and the `M_n=Theta((log n)^2)` budget lie outside the no-go parameter range;
...
- First unresolved theorem node: prove or disprove occupancy in the region beyond the connected-core certified reach, including endpoint support near `Y_n`.

## What is not claimed

This file does not prove the factorial half-range theorem. It does not prove that the connected-core recursion reaches `Y_n`. It does not turn formal profile capacity into occupancy. It does not show that a failure of the sequential carrier criterion is a counterexample to the full rainbow model.